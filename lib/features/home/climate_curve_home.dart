...
class ClimateCurveOfflineHomeState extends State<ClimateCurveOfflineHome> {
  ...
  List<DailyRecordDTO> recordsSinceLastApply(SystemMode mode) {
    final last = mode == SystemMode.heating ? lastAiApplyHeating : lastAiApplyCooling;
    if (last == null) return List.from(records);
  ...
  Future<void> addRecord() async {
    if (externalTempController.text.isEmpty || consumptionController.text.isEmpty) {
      if (mounted) {
    ...
    final double? extTemp = double.tryParse(externalTempController.text.replaceAll(',', '.'));
    final double? cons = double.tryParse(consumptionController.text.replaceAll(',', '.'));
                      ...
                      Row(
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              const Text('Curva Climatica', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E1E1E))),
                              Text(isHeating ? 'Inverno (Riscaldamento)' : 'Estate (Raffrescamento)', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                            ],
                          ),
                          Row(
                            children: [
                              buildLegendItem('Attuale', Colors.blue.shade700, false),
                          ...
                          LineChartData(
                            ...
                            lineTouchData: LineTouchData(
                              touchTooltipData: LineTouchTooltipData(
                                ...
                                getTooltipItems: (touchedBarSpots) => touchedBarSpots.map((barSpot) => LineTooltipItem('Est ${barSpot.x.toInt()}°C: ${barSpot.y.toStringAsFixed(1)}°C', const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12))).toList(),
                              ),
                            ),
                            rangeAnnotations: RangeAnnotations(horizontalRangeAnnotations: [HorizontalRangeAnnotation(y1: minY, y2: unsafeZoneLimit, color: Colors.red.withOpacity(0.10))]),
                            gridData: FlGridData(show: true, drawVerticalLine: true, horizontalInterval: 5, verticalInterval: 5, getDrawingHorizontalLine: (v) => FlLine(color: Colors.grey.withOpacity(0.1), strokeWidth: 1), getDrawingVerticalLine: (v) => FlLine(color: Colors.grey.withOpacity(0.1), strokeWidth: 1)),
                            titlesData: FlTitlesData(
                              leftTitles: AxisTitles(axisNameWidget: const Text('Temp. Mandata Acqua °C', style: TextStyle(fontSize: 10, color: Colors.grey)), axisNameSize: 20, sideTitles: SideTitles(showTitles: true, reservedSize: 35, interval: 5, getTitlesWidget: (val, m) => Text(val.toInt().toString(), style: const TextStyle(fontSize: 11, color: Colors.grey, fontWeight: FontWeight.bold)))),
                              bottomTitles: AxisTitles(axisNameWidget: const Text('Temp. Esterna °C', style: TextStyle(fontSize: 10, color: Colors.grey)), axisNameSize: 20, sideTitles: SideTitles(showTitles: true, interval: 5, getTitlesWidget: (val, m) => Padding(padding: const EdgeInsets.only(top: 6), child: Text(val.toInt().toString(), style: const TextStyle(fontSize: 11, color: Colors.grey))))),
                              topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
                              rightTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
                            ),
                            borderData: FlBorderData(show: true, border: Border.all(color: Colors.grey.shade300)),
                            lineBarsData: [
                              LineChartBarData(spots: currentSpots, isCurved: true, color: Colors.blue.shade700, barWidth: 4, isStrokeCapRound: true, dotData: const FlDotData(show: false), belowBarData: BarAreaData(show: true, color: Colors.blue.withOpacity(0.05))),
                              if (suggestedSpots != null) LineChartBarData(spots: suggestedSpots, isCurved: true, color: Colors.green, barWidth: 3, dashArray: [6, 6], dotData: const FlDotData(show: false)),
                            ],
                          ),
                        ),
                      ),
