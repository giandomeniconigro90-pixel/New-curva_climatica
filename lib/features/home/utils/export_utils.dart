...
class ExportUtils {
  ...
  // === GENERAZIONE CSV CON DATI CURVA ===
  static String generateCsv(List<DailyRecordDTO> records, {
    ...
    // Header esteso con colonna temperatura mandata calcolata
    buffer.writeln('Data;Temp Esterna;T Mandata Calcolata;Consumo;Note');

    for (final r in records) {
      // Calcola la temperatura mandata in base alla curva attuale
  ...
  static Future<String> saveCsv(String csvData, String fileName) async {
  ...
  static Future<void> generateAndSavePdf({
    ...
    // Colori PDF
    final PdfColor primaryColor = isHeating
        ? PdfColor.fromInt(0xFFD84315)
        : PdfColor.fromInt(0xFF0277BD);

    final PdfColor accentColor = isHeating
        ? PdfColor.fromInt(0xFFFF7043)
        : PdfColor.fromInt(0xFF29B6F6);
    ...
    // === PAGINA 1: DATI PRINCIPALI + GRAFICO ===
    pdf.addPage(
        pw.MultiPage(
            ...
            build: (context) => [
              ...
              // GRAFICO (PULITO, SENZA RIQUADRI)
              if (chartImage != null) ...[
                _buildSectionTitle('ANALISI VISIVA', primaryColor),
                pw.SizedBox(height: 15),
                pw.Center(
                  child: pw.Container(
                    height: 300, // Altezza fissa per non sformare
    ...
    // === PAGINA 2: TABELLA RIFERIMENTO + STORICO SULLA STESSA PAGINA ===
    if (filteredRecords.isNotEmpty) {
      pdf.addPage(
          pw.MultiPage(
              ...
              margin: const pw.EdgeInsets.all(40),
              build: (context) => [
                // TABELLA DI RIFERIMENTO (PIÙ PICCOLA)
                ...
                // STORICO RILEVAZIONI (SULLA STESSA PAGINA)
  ...
  static pw.Widget _buildCurveReferenceTableCompact(
      double slope,
      double offset,
      SystemMode mode,
    ...
    final List<int> temps = mode == SystemMode.heating
        ? [-5, 0, 5, 10, 15]
        : [25, 28, 30, 35, 40];
    ...
    return pw.Table(
      ...
      children: rows.asMap().entries.map((entry) {
        ...
        return pw.TableRow(
          decoration: pw.BoxDecoration(
            color: isHeader ? primaryColor : (entry.key % 2 == 0 ? PdfColors.grey50 : PdfColors.white),
          ),
  ...
  static pw.Widget _buildRecordsTableCompact(
    ...
    final dataRows = records.map((r) {
      ...
      return [
        r.dateIso,
        '${r.externalTemp.toInt()}°',
        '${mandata.toStringAsFixed(1)}°',
        '${r.consumption}',
      ];
    }).toList();
